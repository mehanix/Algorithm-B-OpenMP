...
class Graph {
	...
	public:
		...
		static void Dijkstra(const Graph& graph, Vertex start);
        static void AStarSearch(const Graph& graph, Vertex start, Vertex goal);
		static unsigned int heuristic(const Graph& graph, Vertex node, Vertex goal);
...
class ParallelGraphAlgorithms {
	...
	public:
		...
		*/
		...
		struct bfs_t {
			...
			bfs_t(unsigned n): _p(n,-1), _d(n,-1)	{}
		};
	...
	*/
	...
	public:
        ...
        static Graph algorithm_R_spanning_forest(Graph,vector<unsigned int>&,vector<unsigned int>&);
		static void algorithm_A(Graph,vector<unsigned int>&,vector<unsigned int>&);
		static void algorithm_P(Graph,vector<unsigned int>&,vector<unsigned int>&);
        static void algorithm_SRT(Graph,vector<unsigned int>&,vector<unsigned int>&,vector<unsigned int>&);
		static void algorithm_H(Graph,vector<unsigned int>&,vector<unsigned int>&,vector<unsigned int>&);
		static void algorithm_Reif(Graph G, vector<unsigned int>& p, vector<unsigned int>& o);
        static void algorithm_B(Graph G_orig, vector<unsigned int>& p);
	...
	private:
        ...
        static bool randomized_parent_connect(Graph& G, vector<unsigned int>& p, vector<unsigned int>& o, vector<unsigned int>& coin);
        static bool direct_root_connect_forest(Graph& G, vector<unsigned int>& p, vector<unsigned int>& o,Graph& forest);
    ...
    public:
        static void FWBW(Graph&, unsigned int n);
        static Graph generate_transpose(Graph& G);
        static Graph induced_subgraph_difference(Graph& G, vector<int>&,vector<int>&, unsigned int);
        static Graph induced_subgraph(Graph& G, vector<int>&, unsigned int);
        static void coloring(Graph& G, unsigned int n);
        static void simple_pbfs_color(unsigned int s, Graph G, ParallelGraphAlgorithms::bfs_t& T, vector<int>& vertex_colors, int color);
